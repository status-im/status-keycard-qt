...
FlowManager::FlowManager(QObject* parent)
    ...
    , m_currentCardUid("")
{
    qDebug() << "StatusKeycardQt::FlowManager: Created";
}
...
bool FlowManager::init(std::shared_ptr<Keycard::ICommunicationManager> commMgr)
{
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Connecting to CommunicationManager card lifecycle events";

    connect(m_commMgr.get(), &Keycard::ICommunicationManager::cardInitialized,
            this, [this](Keycard::CardInitializationResult result) {
                if (result.success) {
                    qDebug() << "StatusKeycardQt::FlowManager: Card initialized and ready, UID:" << result.uid;
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Initialized successfully with CommunicationManager";
    qDebug() << "StatusKeycardQt::FlowManager: CommunicationManager:" << (m_commMgr ? "YES" : "NO");
    return true;
}
...
bool FlowManager::startFlow(int flowType, const QJsonObject& params)
{
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Starting flow type:" << flowType << "params:" << params;
...
bool FlowManager::resumeFlow(const QJsonObject& params)
{
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Resuming flow";
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Flow resumed";
    return true;
}
...
bool FlowManager::cancelFlow()
{
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Cancelling flow";
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Flow cancelled";
    return true;
}
...
void FlowManager::onCardDetected(const QString& uid)
{
    ...
    if (m_currentCardUid == uid) {
        qDebug() << "StatusKeycardQt::FlowManager: Same card, ignoring";
    ...
    if (m_waitingForCard && m_currentFlow) {
        ...
        qDebug() << "StatusKeycardQt::FlowManager: Not resuming (not waiting or no current flow)";
    }
}

void FlowManager::onCardRemoved()
{
    qDebug() << "StatusKeycardQt::FlowManager: Card removed";

#if defined(Q_OS_ANDROID) || defined(Q_OS_IOS)
    qDebug() << "StatusKeycardQt::Ignoring card removals";
    return;
#else
...
{
    qDebug() << "StatusKeycardQt::FlowManager: Flow paused, action:" << action;
...
{
    qDebug() << "StatusKeycardQt::FlowManager: Flow completed successfully";
    // Emit result signal
...
{
    qDebug() << "StatusKeycardQt::FlowManager: Creating flow type:" << static_cast<int>(flowType);
...
{
    qDebug() << "StatusKeycardQt::FlowManager: Running flow asynchronously";
    ...
    m_flowFuture = QtConcurrent::run([this]() {
        ...
        do {
            ...
            if (shouldRestart) {
                qDebug() << "StatusKeycardQt::FlowManager: Restarting flow from beginning";
            ...
            try {
                ...
                // Check if cancelled
                if (flow->isCancelled()) {
                    qDebug() << "StatusKeycardQt::FlowManager: Flow was cancelled";
                ...
                if (shouldRestart) {
                    qDebug() << "StatusKeycardQt::FlowManager: Flow requested restart (card swap)";
                    // Loop will restart execution
                } else {
                    // Flow completed successfully
                    qDebug() << "StatusKeycardQt::FlowManager: Flow execution completed";
        ...
        qDebug() << "StatusKeycardQt::FlowManager: Flow loop exited";
    });
}

void FlowManager::cleanupFlow()
{
    qDebug() << "StatusKeycardQt::FlowManager: Cleaning up flow";

    // Wait for async flow execution to complete before cleaning up
    if (m_flowFuture.isValid() && !m_flowFuture.isFinished()) {
        qDebug() << "StatusKeycardQt::FlowManager: Waiting for async flow to finish...";
        m_flowFuture.waitForFinished();
        qDebug() << "StatusKeycardQt::FlowManager: Async flow finished";
    }
    ...
    qDebug() << "StatusKeycardQt::FlowManager: Cleanup complete";
}
...
QJsonObject FlowManager::buildCardInfoFromCommandSet() const
{
    ...
    try {
        ...
        qDebug() << "StatusKeycardQt::FlowManager: Could not get card info from CommandSet";
    }
