...
QJsonObject RecoverAccountFlow::execute()
{
    ...
    qDebug() << "StatusKeycardQt::RecoverAccountFlow: Execution completed successfully";
    return result;
}
...
QJsonObject RecoverAccountFlow::exportKey(const QString& path, bool includePrivate)
{
    ...
    qDebug() << "StatusKeycardQt::RecoverAccountFlow::exportKey() - Export SUCCESS for path:" << path;
    ...
    qDebug() << "StatusKeycardQt::RecoverAccountFlow: Key exported successfully for path:" << path;
    return keyPair;
}
