...
{
    qDebug() << "StatusKeycardQt::FlowBase::selectKeycard()";
    ...
    qDebug() << "StatusKeycardQt::FlowBase::selectKeycard() - SUCCESS";
    return true;
}
...
FlowResult FlowBase::initializeKeycard()
{
    ...
    qDebug() << "StatusKeycardQt::FlowBase::initializeKeycard() - SUCCESS";
    return FlowResult{true, buildCardInfoJson()};
}

bool FlowBase::unblockPIN()
{
    qDebug() << "StatusKeycardQt::FlowBase: Unblocking PIN...";
...
{
    qDebug() << "StatusKeycardQt::FlowBase: Verifying PIN...";
    ...
    qDebug() << "StatusKeycardQt::FlowBase: PIN verified successfully";
    return true;
}
...
bool FlowBase::requireKeys()
{
    if (!cardInfo().keyUID.isEmpty()) {
        qDebug() << "StatusKeycardQt::FlowBase: Card has keys";
        return true;
    }
...
FlowResult FlowBase::requireNoKeys()
{
    QJsonObject result = buildCardInfoJson();
    if (cardInfo().keyUID.isEmpty()) {
        qDebug() << "StatusKeycardQt::FlowBase: Card has no keys (as required)";
        return {true, result};
    }
    ...
    if (m_params.contains(FlowParams::OVERWRITE) &&
        ...
        qDebug() << "StatusKeycardQt::FlowBase: Card has keys but overwrite allowed";
        return {true, result};
    }
...
FlowResult FlowBase::loadMnemonic()
{
    ...
    qDebug() << "StatusKeycardQt::FlowBase::loadMnemonic() - SUCCESS";
